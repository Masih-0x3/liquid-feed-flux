...
function withFixture(callback) {
  ...
  try {
    ...
    cpSync(join(REPO_ROOT, "supabase/migrations"), join(root, "supabase/migrations"), { recursive: true });
...
test("the committed XOT migration inventory validates without network access", () => {
  const result = validateMigrationBaseline();
  assert.equal(result.observedSideEntries, 210);
  assert.equal(result.activeMigrations, 107);
  assert.equal(result.remoteSnapshotChecked, false);
  assert.equal(result.releaseReady, false);
  assert.deepEqual(result.blockers, REQUIRED_BLOCKERS);
});
...
test("active source tampering is rejected", () =>
  withFixture((root) => {
    const migration = join(root, "supabase/migrations/20250902164607_4824a25b-3b01-4499-8ddd-2dd4e191920b.sql");
    writeFileSync(migration, `${readFileSync(migration, "utf8")}\n-- tampered\n`);
    assert.throws(() => validateMigrationBaseline({ root }), /active migration hash changed for 20250902164607/);
  }));

test("coordinated active hash rewrites still violate immutable observation", () =>
  withFixture((root) => {
    const migration = join(root, "supabase/migrations/20250902164607_4824a25b-3b01-4499-8ddd-2dd4e191920b.sql");
    ...
    assert.throws(
      ...
      /active source differs from immutable observed local body for 20250902164607/,
    );
  }));
...
test("retired alias archive tampering is rejected", () =>
  withFixture((root) => {
    const archive = join(root, ARCHIVED_ALIAS_PATH);
    writeFileSync(archive, `${readFileSync(archive, "utf8")}\n-- tampered\n`);
    assert.throws(() => validateMigrationBaseline({ root }), /archived alias does not match the immutable observed source hash/);
  }));

test("duplicate active migration versions are rejected", () =>
  withFixture((root) => {
    cpSync(
      join(root, "supabase/migrations/20250902164607_4824a25b-3b01-4499-8ddd-2dd4e191920b.sql"),
...
test("renaming a retained migration with the same body is rejected", () =>
  withFixture((root) => {
    ...
    assert.throws(
      () => validateMigrationBaseline({ root }),
      /active migration filenames\/body inventory differs from protected candidate input/,
    );
  }));
...
test("checked-in type hash is recomputed", () =>
  withFixture((root) => {
    const types = join(root, "src/integrations/supabase/types.ts");
    writeFileSync(types, `${readFileSync(types, "utf8")}\n// tampered\n`);
    assert.throws(() => validateMigrationBaseline({ root }), /checked-in Supabase type hash differs from the manifest/);
  }));
