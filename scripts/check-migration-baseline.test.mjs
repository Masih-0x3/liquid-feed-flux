...
// Keep the standalone current-candidate fixtures anchored to the immutable
// pre-v2 migration body. CI checks out only the tested commit, so HEAD is the
// repaired body and cannot be used as a predecessor source.
const PRE_V2_MIGRATION_SOURCE = String.raw`-- E7 / AIR-009: remove browser-facing defaults from every public-schema
...
assert.equal(
  sha256(PRE_V2_MIGRATION_SOURCE),
  "103e8c1a608b120d8945296385583814e748e92904882683cf0610e7a9130d7a",
  "pre-v2 migration fixture must remain byte-identical to the frozen baseline",
);
...
test("append-only successor baseline fails when a seventh tail migration is present (135 -> 136)", () =>
  withCurrentTreeFixture((root) => {
...
test("successor baseline keeps the historical 129 receipt and exact 135-file current tree", () =>
  withCurrentTreeFixture((root) => {
    const result = validateMigrationBaseline({ root });
    assert.equal(result.currentCandidateActiveCount, CURRENT_ON_DISK_MIGRATION_COUNT);
    assert.equal(result.currentCandidateChecked, true);
