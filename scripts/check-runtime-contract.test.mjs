...
test("comments and echo text cannot impersonate exact CI runtime steps", () => withFixture((root) => {
  ...
  const value = readFileSync(path, "utf8")
    .replace("      - run: npm run check:runtime-contract", "      # run: npm run check:runtime-contract")
    .replace("      - run: npm run test:runtime-contract", "      - run: echo npm run test:runtime-contract");
  writeFileSync(path, value);
  assert.ok(validateRuntimeContract({ root, actualNodeVersion: "20.19.0" }).errors.some((error) => error.includes("exact runtime contract run step")));
}));
