...
Production status as of 2026-06-15T19:59Z: steps 1-4 below are complete. A signed webhook request without a query token returned HTTP 200, and an unsigned query-token-only request returned HTTP 401. Steps 5-7 remain open before query-token code can be deleted.
...
6. Rotate the exposed old query-token secret and regenerate the RSS.app signing secret if it has appeared in chat, screenshots, logs, or docs. Store the regenerated signing secret in Supabase before sending another signed test.
