...
- PR #51 added and deployed RSS.app signed-webhook verification from `257c69f047971683c22ca57df4cf137c8d89a8c7`; RSS.app signing was activated, `RSSAPP_SIGNING_SECRET` was stored, and `RSSAPP_ALLOW_QUERY_TOKEN=false` was set. A signed no-query webhook smoke returned HTTP 200, an unsigned query-token-only request returned HTTP 401, and compatibility-code deletion remains gated on removing/rotating the old URL token and a zero-hit quiet window.
