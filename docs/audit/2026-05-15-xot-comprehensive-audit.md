...
| P1 | Secrets | RSS webhook token is logged in Supabase Edge Function URLs. | Edge logs include `webhooks-rssapp?token=[REDACTED]`. Local auth accepts query tokens in `supabase/functions/_shared/internalAuth.ts:45`. | Rotate token, use `x-webhook-token`/`x-rssapp-token` headers only, reject query tokens after migration, scrub logs. |
