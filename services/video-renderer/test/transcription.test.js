...
for (const contextText of [undefined, null, "", "  \n  "]) {
  test(`keeps sparse confident speech without context (${JSON.stringify(contextText)})`, async () => {
    const segments = [{ id: 1, start: 4.7, end: 5.5, text: "Olive grove." }];
    assert.equal(isSparseContextMismatchedTranscript({ segments }, { durationMs: 10000, contextText }), false);
    const result = await transcribeAudio({
      provider: "deepgram", audioPath: "/tmp/audio.mp3", durationMs: 10000,
      deepgramApiKey: "test-key", contextText,
      fetchImpl: async () => ({ ok: true, text: async () => JSON.stringify({ results: {
        channels: [{ detected_language: "en", language_confidence: 0.95,
          alternatives: [{ transcript: "Olive grove.", confidence: 0.95,
            words: [{ word: "Olive", start: 4.7, end: 5.1, confidence: 0.95 }, { word: "grove", start: 5.1, end: 5.5, confidence: 0.95 }],
          }],
        }],
        utterances: [{ start: 4.7, end: 5.5, transcript: "Olive grove." }],
      } }) }),
