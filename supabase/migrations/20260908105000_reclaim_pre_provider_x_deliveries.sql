...
-- released, due, pre-provider `pending` row is no longer excluded, opening the auto-cron reclaim
-- path. The in-loop and `existing`-set gates in `supabase/functions/x-poster/index.ts`
-- are updated in lockstep. Ordinary pending and not-yet-due rows remain excluded
-- BEFORE LIMIT so they cannot starve eligible work.
BEGIN;
  ...
  WHERE (target_tweet_id IS NULL OR p.tweet_id=target_tweet_id)
    ...
    AND NOT EXISTS (SELECT 1 FROM public.x_deliveries xd WHERE xd.post_id=p.tweet_id
      AND (xd.status IN ('posted','skipped','failed','posting','running')
        OR (xd.status = 'pending' AND (
          xd.claim_release_reason IS DISTINCT FROM 'pre_provider_retry'
          OR xd.claim_token IS NOT NULL OR xd.provider_started_at IS NOT NULL
          OR xd.x_tweet_id IS NOT NULL OR xd.next_retry_at > now()
        ))))
    AND NOT EXISTS (
