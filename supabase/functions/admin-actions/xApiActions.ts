...
export async function sendTestTweetAdminAction(
  ...
  if (!isMyXEnabled(asRecord(asRecord(controlsRow).value))) {
    return {
      body: {
        ok: false,
        disabled: true,
        reason: "owned_writes_disabled",
        error: "Owned X writes are disabled by x_api_controls.",
      },
      status: 200,
    };
  }
  ...
  try {
    ...
    if (!resp.ok) {
      const errorCode = xApiFailureCode("x_test_tweet", resp.status);
      return {
        body: {
          ok: false,
          error: errorCode,
    ...
    const errorCode = "x_test_tweet_request_failed";
    await recordAdminXApiAttempt(supabase, {
      action: "send_test_tweet",
      endpoint: url,
      method: "POST",
