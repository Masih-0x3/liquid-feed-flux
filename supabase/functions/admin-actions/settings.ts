...
export function shouldRestampXPostingStart(prevCfg: Record<string, unknown>, nextCfg: Record<string, unknown>): boolean {
  ...
  return !userProvidedStart && (enableTransition || thresholdLowered || mediaLoosened || decisionGateLoosened);
}
...
export async function saveSettingsAdminAction(
  ...
  if (!SETTINGS_ALLOWED_KEYS.includes(key as typeof SETTINGS_ALLOWED_KEYS[number])) {
    return { body: { error: `Setting key "${key}" is not allowed` }, status: 400 };
  }
  ...
  if (key === 'x_posting_config' && value && typeof value === 'object') {
    ...
    const { data: prev } = await settings.select('value').eq('key', 'x_posting_config').maybeSingle();
    ...
    if (shouldRestampXPostingStart(prevCfg, nextCfg)) {
      ...
      console.log('[admin-actions] re-stamped x_posting_config.start_posting_from', {
        ...
        thresholdLowered: !!nextCfg.enabled && typeof nextCfg.min_score === 'number' && nextCfg.min_score < (typeof prevCfg.min_score === 'number' ? prevCfg.min_score : 14),
        mediaLoosened: !!nextCfg.enabled && prevCfg.require_media === true && nextCfg.require_media === false,
        decisionGateLoosened: !!nextCfg.enabled && prevCfg.post_only_decision_deliver === true && nextCfg.post_only_decision_deliver === false,
        prevMin: typeof prevCfg.min_score === 'number' ? prevCfg.min_score : 14,
        nextMin: typeof nextCfg.min_score === 'number' ? nextCfg.min_score : (typeof prevCfg.min_score === 'number' ? prevCfg.min_score : 14),
        prevStart: typeof prevCfg.start_posting_from === 'string' ? prevCfg.start_posting_from : null,
        nextStart: typeof nextCfg.start_posting_from === 'string' ? nextCfg.start_posting_from : null,
      });
    }
  }

  const settings = supabase.from('settings') as SettingsQueryBuilder;
  const { error } = await settings.upsert({ key, value: valueToSave, updated_at: new Date().toISOString() }, { onConflict: 'key' });
