import type { AdminActionResponse, RecordFeedbackFn, SupabaseAdminClient } from "./types.ts";

type MutationResult = { data?: Array<Record<string, unknown>> | null; error?: unknown };
...
export async function editTranslationAdminAction(
  ...
  if (!tweet_id || text_translated === undefined) {
    return { body: { error: 'tweet_id and text_translated are required' }, status: 400 };
  }
  const { error } = await table(supabase, 'posts')
    .update({ text_translated })
    .eq('tweet_id', tweet_id);
  if (error) throw error;
  await recordFeedback(supabase, tweet_id as string, 'edit_translation', 0).catch(() => {});
...
export async function retryStepAdminAction(
  ...
  if (error) throw error;
  if (step === 'deliver') {
    await recordFeedback(supabase, tweet_id as string, 'force_deliver', 2).catch(() => {});
    await table(supabase, 'posts').update({ feedback_locked: true }).eq('tweet_id', tweet_id);
  }
...
export async function cancelPendingJobsAdminAction(
  ...
  return { body: { success: true, canceled, by_type: byType, message: `Canceled ${canceled} job(s)` } };
}
...
export async function bulkReprocessAdminAction(
  ...
  const tweetIds = [
    ...new Set(tweet_ids.map((tid: unknown) => typeof tid === 'string' ? tid.trim() : '').filter(Boolean)),
  ] as string[];
  ...
  const { error } = await table(supabase, 'jobs').upsert(jobs, { onConflict: 'idempotency_key', ignoreDuplicates: true });
  if (error) throw error;
  return {
    body: {
      success: true,
...
export async function getHealthAdminAction(supabase: SupabaseAdminClient): Promise<AdminActionResponse> {
  const { data, error } = await supabase.rpc('get_system_health');
  if (error) throw error;
  return { body: { success: true, health: data } };
}

export async function reconcileStuckJobsAdminAction(supabase: SupabaseAdminClient): Promise<AdminActionResponse> {
