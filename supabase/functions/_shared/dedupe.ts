...
export type DedupeStatus = 'pending' | 'unique' | 'duplicate' | 'related_new_info' | 'uncertain' | 'coverage_gap' | 'failed' | 'disabled';
export type DedupeMethod = 'none' | 'exact_tweet' | 'exact_url' | 'semantic_auto' | 'semantic_ai';
export type DedupeFailurePhase = 'embedding' | 'candidate_lookup' | 'coverage_check' | 'post_update' | 'event_insert' | 'signature_upsert' | 'adjudicator' | 'unknown';
...
export function normalizeDuplicateGateConfig(raw: unknown): DuplicateGateConfig {
  ...
  const windowHours = typeof input.window_hours === 'number'
    ? Math.max(DEFAULT_DUPLICATE_GATE.window_hours, Math.min(168, Math.round(input.window_hours)))
...
export async function runDuplicateGate(
  ...
  try {
    ...
    if (!dryRun) await upsertBareStorySignature(supabase, post, embeddingLiteral);
    ...
    if (candidates.length === 0) {
      ...
      if (config.mode === 'semantic_only') {
        ...
        const canonical = await selectBestCanonicalCandidate(supabase, candidates, config.similarity_threshold);
        result = baseResult({
          status: 'duplicate',
          method: 'semantic_auto',
          confidence: top.similarity,
    ...
    if (!dryRun) {
      ...
      await insertDedupeEvent(supabase, post.tweet_id, 'failed', buildDedupeEventMeta(result, config, options.source, post.feedback_locked === true, {
        error: message,
        failure_phase: failure.phase,
        retryable: failure.retryable,
      }));
...
export async function assertFinalDuplicateState(
  ...
  if (!config.enabled || config.action !== 'skip') {
    return { checked: false, blocked: false, reason: 'duplicate_gate_disabled', result: null };
  }

  const nowIso = new Date().toISOString();
  const { data: post, error } = await supabase
    .from('posts')
    .select('tweet_id, text_original, text_translated, author_handle, url, created_at, delivery_decision, decision_reason, feedback_locked, dedupe_status, dup_of_tweet_id, dedupe_reason, dedupe_confidence, dedupe_method, dup_similarity, story_cluster_id')
    .eq('tweet_id', tweetId)
    .maybeSingle();
  if (error || !post) {
    return { checked: true, blocked: false, reason: `post_lookup_failed:${error?.message ?? 'not_found'}`, result: null };
  }
  ...
  if (postRecord.dedupe_status === 'related_new_info') {
    return { checked: true, blocked: false, reason: 'related_new_info', result: null };
  }
  if (postRecord.dedupe_status === 'coverage_gap' && postRecord.dup_of_tweet_id) {
    const coverage = await loadDuplicateCoverage(supabase, postRecord.dup_of_tweet_id);
    if (coverage.safe_to_block) {
      const result: DuplicateGateResult = {
        ...
        reason: `coverage_gap_resolved:${coverage.reason}; ${postRecord.dedupe_reason ?? ''}`.slice(0, 1200),
        new_facts: [],
        should_enqueue_translate: false,
        candidates: [],
      };
      if (!dryRun) {
        await persistDedupeResult(supabase, postRecord, result, config, nowIso, options.source ?? 'final_assertion');
      }
      return { checked: true, blocked: true, reason: result.reason, result };
    }
    return { checked: true, blocked: false, reason: `coverage_gap:${coverage.reason}`, result: null };
  }
  if (postRecord.dedupe_status === 'uncertain') {
    return { checked: true, blocked: false, reason: 'uncertain_duplicate_review', result: null };
  }

  const text = (postRecord.text_translated || postRecord.text_original || '').trim();
  if (!text || normalizeStoryText(text).length < 20) {
    return { checked: true, blocked: false, reason: 'too_little_text', result: null };
  }
  ...
  const { data: signature } = await supabase
    ...
    .eq('tweet_id', tweetId)
    .maybeSingle();
  embeddingLiteral = toEmbeddingLiteral((signature as { embedding?: unknown } | null)?.embedding);
  ...
  if (!embeddingLiteral) {
    try {
      ...
      if (!dryRun) await upsertBareStorySignature(supabase, postRecord, embeddingLiteral);
    } catch (e) {
      return {
        checked: true,
        blocked: false,
  ...
  const candidates = await findSemanticCandidates(supabase, embeddingLiteral, tweetId, config);
  const top = candidates[0];
  if (!top || top.similarity < config.similarity_threshold) {
    return { checked: true, blocked: false, reason: top ? `below_final_duplicate_threshold:${top.similarity.toFixed(3)}` : 'no_semantic_candidates', result: null };
  }
  ...
  if (top.similarity >= config.auto_duplicate_similarity) {
    ...
    result = {
      ...
      story_cluster_id: canonical.story_cluster_id ?? top.story_cluster_id,
      similarity: top.similarity,
      reason: `final_assertion_high_semantic_similarity:${top.similarity.toFixed(3)}`,
      new_facts: [],
      should_enqueue_translate: false,
      candidates,
    };
    ...
    const key = Deno.env.get('OPENAI_API_KEY') ?? '';
    if (!key) {
      return { checked: true, blocked: false, reason: 'final_assertion_openai_key_missing', result: null };
    }
    result = await adjudicateWithModel(key, postRecord, candidates, config);
    if (result.status === 'unique') {
      result = { ...result, reason: `final_assertion_distinct:${result.reason}` };
    }
  }

  result = canonicalizeDedupeResult(result, candidates);
  result = await canonicalizeResultThroughPosts(supabase, result);
  result = await preventUncoveredDuplicateSkip(supabase, postRecord, result, config);

  if (result.status === 'duplicate' || result.status === 'coverage_gap' || result.status === 'related_new_info' || result.status === 'uncertain') {
    if (!dryRun) {
      await persistDedupeResult(supabase, postRecord, result, config, nowIso, options.source ?? 'final_assertion');
    }
  }
...
async function findSemanticCandidates(
  ...
  return (data ?? []).map((row: Record<string, unknown>) => ({
    ...
    candidate_final_score: typeof row.candidate_final_score === 'number' ? row.candidate_final_score : Number(row.candidate_final_score ?? NaN) || null,
    candidate_importance_score: typeof row.candidate_importance_score === 'number' ? row.candidate_importance_score : Number(row.candidate_importance_score ?? NaN) || null,
  }));
}

function toEmbeddingLiteral(value: unknown): string | null {
  if (typeof value === 'string' && value.trim().startsWith('[')) return value.trim();
  if (Array.isArray(value) && value.every((v) => typeof v === 'number')) return `[${value.join(',')}]`;
  return null;
}
...
async function selectBestCanonicalCandidate(
  ...
  const eligible = candidates.filter((candidate) => candidate.similarity >= minSimilarity);
    ...
    const coverageRank = coverage.state === 'delivered'
      ...
      : coverage.state === 'active_pipeline'
        ...
        : coverage.state === 'coverage_gap'
          ? 1
          : 0;
    const score = candidate.candidate_final_score ?? candidate.candidate_importance_score ?? 0;
    const created = candidate.created_at ? new Date(candidate.created_at).getTime() : Number.MAX_SAFE_INTEGER;
...
async function upsertStorySignature(
  ...
  const { error } = await supabase.from('story_signatures').upsert(payload, { onConflict: 'tweet_id' });
...
async function persistDedupeResult(
  ...
  if ((result.status === 'duplicate' || result.status === 'uncertain' || result.status === 'coverage_gap') && result.dup_of_tweet_id) {
...
function thresholdSnapshot(config: DuplicateGateConfig): DedupeThresholdSnapshot {
  return {
...
function summarizeCandidates(candidates: StoryCandidate[]): Array<Record<string, unknown>> {
...
function serializeCoverage(coverage: DuplicateCoverage): Record<string, unknown> {
  return {
...
function classifyDedupeFailure(message: string): { phase: DedupeFailurePhase; retryable: boolean } {
  if (message.startsWith('embedding_error:') || message === 'OPENAI_API_KEY is not configured') {
    return { phase: 'embedding', retryable: message.startsWith('embedding_error:') && isRetryableProviderError(message) };
  }
  if (message.startsWith('find_story_candidates_v3_failed:') || message.startsWith('exact_url_lookup_failed:')) {
  ...
  if (message.startsWith('dedupe_ai_error:')) {
    return { phase: 'adjudicator', retryable: isRetryableProviderError(message) };
  }
...
function isRetryableProviderError(message: string): boolean {
  return /(^|:)(408|409|425|429|500|502|503|504)(:|$)/.test(message)
    || /timeout|timed out|temporarily|unavailable|rate limit|ECONNRESET|network/i.test(message);
}
