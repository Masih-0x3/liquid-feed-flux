...
Deno.test("media cleanup workflow dry-run performs selection reads without storage or database mutation", async () => {
  ...
  const client = {
    rpc: async (name: string) => {
      ...
      if (name === "get_old_media") {
        return await Promise.resolve({
          data: [{ id: "media-1", storage_path: "2026/7/media.jpg" }],
          error: null,
        });
      }
  ...
  assertEquals(response.status, 200);
  assertEquals(rpcCalls, ["get_old_media", "get_expired_video_render_paths"]);
