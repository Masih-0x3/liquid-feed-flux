...
import { isArchivePostIdentity, matchesArchivePostReference } from '../../supabase/functions/_shared/archivePostIdentity';
export { parseArchivePostReference } from '../../supabase/functions/_shared/archivePostIdentity';
...
export const MEDIA_ACCESS_MESSAGES: Record<string, string> = {
  ...
  media_post_ambiguous: 'This reference matches multiple archive records. Open the exact render in Video or ask an administrator to review the duplicate records.',
...
export function validateMediaGrant(
  ...
  if (!isArchivePostIdentity(target.tweetId) || grant.id !== (target.renderId || target.mediaId) || grant.tweet_id !== target.tweetId
...
type MediaCatalog =
  | { ok: true; tweet_id: string; assets: ArchivedAsset[]; author_handle?: string }
  | { ok: false; code?: string; assets: ArchivedAsset[] };

export async function getMediaCatalog(tweetId: string): Promise<MediaCatalog> {
  ...
  if (!matchesArchivePostReference(response.tweet_id, tweetId) || !Array.isArray(response.assets) || response.assets.length > 24
  ...
  return {
    ok: true, tweet_id: response.tweet_id, author_handle: typeof response.author_handle === 'string' ? response.author_handle : undefined,
