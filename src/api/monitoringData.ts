...
export type MonitoringDataSource = 'admin_actions' | 'legacy_queries';
...
function isMissingMonitoringColumnError(error: unknown): boolean {
...
export async function fetchMonitoringEntriesViaAdminActions({
  pageParam = 0,
  filter,
  search,
  scoreBucket,
...
export async function fetchMonitoringEntriesViaLegacyQueries(
  params: MonitoringFetchParams,
  adminActionError?: unknown,
): Promise<MonitoringPage> {
  const { pageParam = 0, filter, search, scoreBucket } = params;
  ...
  if (legacyFilter === 'failed' || legacyFilter === 'awaiting-review') {
    ...
    if (!tweetIds || tweetIds.length === 0) {
      return { entries: [], nextCursor: null, source: 'legacy_queries', fallbackReason: adminActionError ? errorMessage(adminActionError) : undefined };
    }
  }

  const term = sanitizeMonitoringSearch(search);
  ...
  if (postsError && isMissingMonitoringColumnError(postsError)) {
  ...
  return {
    ...
    source: 'legacy_queries',
    fallbackReason: adminActionError ? errorMessage(adminActionError) : undefined,
  };
}
...
export async function fetchMonitoringEntriesWithLegacyFallback(
  ...
  try {
    return await fetchMonitoringEntriesViaAdminActions(params);
  } catch (error) {
    return fetchMonitoringEntriesViaLegacyQueries(params, error);
  }
}
