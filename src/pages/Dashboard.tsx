...
export default function Dashboard() {
  ...
  const { metrics, health, activities, heartbeat, opsStatus, pipelineCounts, queueBreakdown, xLocalUsage } = data;
    ...
    <div className="space-y-4 animate-fade-in-up">
      ...
      <Tabs defaultValue="activity" className="space-y-3">
              ...
              <CardContent className="space-y-3">
                ...
                <div className="space-y-2">
                  {queueBreakdown.byType.length > 0 ? queueBreakdown.byType.map((row) => (
                    <div key={row.type} className="grid grid-cols-[minmax(0,1fr)_auto_auto_auto] items-center gap-3 rounded-md border border-border/60 px-3 py-2 text-sm">
        ...
        <TabsContent value="controls">
          <DashboardHealth health={health} queue={queueBreakdown} xUsage={xLocalUsage} />
        </TabsContent>
      </Tabs>
    </div>
  );
