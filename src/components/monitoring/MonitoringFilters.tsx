  ...
  return (
    <CardHeader className="p-3">
      <div className="grid gap-3 xl:grid-cols-[auto_minmax(0,1fr)] xl:items-center">
        ...
        <div className="grid min-w-0 gap-2 sm:grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(10rem,13rem)_minmax(8rem,10rem)] xl:justify-end">
          <div className="relative min-w-0">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
            <Input
              id="monitoring-post-search"
              aria-label="Search monitoring posts"
              value={searchTerm}
          ...
          <ThemedSelect value={filter} onValueChange={(value) => onFilterChange(value as MonitoringFilter)}>
            <SelectTrigger aria-label="Monitoring state filter" className="w-full"><SelectValue /></SelectTrigger>
            <SelectContent>
          ...
          <ThemedSelect value={scoreBucket} onValueChange={(value) => onScoreBucketChange(value as ScoreBucket)}>
            <SelectTrigger aria-label="Score range filter" className="w-full"><SelectValue /></SelectTrigger>
            <SelectContent>
